...
3.  **Mission Awareness**: Create agent/agents base on `src/agents.md` for which the task goal is defined in `.agent/mission.md` (an example mission by default). Align all your actions with this mission or update it to match the project goal.
4.  **Tool-Centric Architecture**: Agents interact with the world through tools. Prioritize creating robust, well-documented tools in the `src/tools/` directory.
