# 🧠 AI-Optimized Project Context: AI Workspace Template

## 1. Executive Summary & Core Mission

**Project Name:** AI Workspace Template
...
1.  **Think (Plan):** Before any complex job, the agent MUST generate a plan in `artifacts/plan_[workflow_id].md` . The workflow_id is a unique identifier for the workflow which is generated, in `.agent/workflows/[workflow_id].md` by the agent based on the tasks. This enforces structured thinking.
...
*   **Mandatory Directives:**
    *   **Read `src/mission.md`:** Before any task, the agent MUST read this file to align with the high-level project objective.
...
*   **Infinite Memory Engine (`src/memory.py` creat it if not exist):** A JSON-based memory manager that uses recursive summarization to manage conversation history, ensuring context is never lost. While less critical for models with large context windows like Gemini 1.5 Pro, it remains a core feature for context compression and management.

### 3.2. Multi-Agent Swarm Mode ( `src/agents/`)
...
*   **`RouterAgent` :** The "manager" or "orchestrator." It receives the user's high-level task, breaks it down into sub-tasks, and delegates them to the appropriate specialist agent. It is also responsible for synthesizing the final result.
...
2.  **Prioritize Planning:** For any request that involves code changes, your first step is to **create or update a workflow/plan** in the `artifacts/` directory.
3.  **Use Your Tools:** Do not perform external actions directly. Use the provided tools in `.agent/tools/` or create new ones if necessary.
